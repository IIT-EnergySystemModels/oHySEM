...
print('\nSoftware for Optimizing Hybrid Systems for Energy and Market management (oHySEM) - Version 1.0.4 - September 11, 2024')
...
def data_processing(DirName, CaseName, model):
    ...
    model.re   = Set(initialize=model.g                                            , ordered=False, doc='RES             units ', filter=lambda model, value: value in model.g           and  parameters_dict['pGenConstantVarCost']      [value] == 0.0 and  parameters_dict['pGenMaximumStorage'][g]       == 0.0  and parameters_dict['pGenProductionFunction'] [g] == 0.0)
    model.es   = Set(initialize=model.g                                            , ordered=False, doc='ESS             units ', filter=lambda model, value: value in model.g           and  parameters_dict['pGenMaximumStorage']       [value]  > 0.0 and (parameters_dict['pVarMaxInflows'].sum()[g]  >  0.0  or  parameters_dict['pVarMaxOutflows'].sum()[g] > 0.0 or parameters_dict['pGenMaximumCharge'][g] > 0.0))
    ...
    model.hz   = Set(initialize=model.h                                            , ordered=False, doc='electrolyzer    units ', filter=lambda model, value: value in model.h           and                                                               parameters_dict['pGenProductionFunction'][h]   >  0.0)
